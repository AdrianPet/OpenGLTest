...
TextureManager::TextureManager()
{
	...
	rapidxml::file<> xmlFile("../data/Player/player.xml"); // Default template is char
	rapidxml::xml_document<> doc;
	doc.parse<0>(xmlFile.data());
	rapidxml::xml_node<> *pRoot = doc.first_node();

	rapidxml::xml_attribute<> *img = pRoot->first_attribute("../data/Player/player.png");
	assert(img);
	...
	{
		std::string locationTex("../data/Player/player/");
		locationTex.append(imageName);
		int width, height, numComponents;
		unsigned char* texture = stbi_load(locationTex.c_str(), &width, &height, &numComponents, 4);
		...
		needed_img->texture = texture;
		needed_img->imageName = imageName;
		needed_img->height = height;
		needed_img->width = width;
	...
	for (rapidxml::xml_node<> *pNode = pRoot->first_node("SubTexture"); pNode; pNode = pNode->next_sibling())
	{
		...
		new_entry->refTex = needed_img->texture;
